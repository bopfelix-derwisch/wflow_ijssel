...
function Clock(config, reader)
    ...
    # Use eltype(nctimes) so starttime has a concrete type (already compiled by NCReader)
    # rather than the abstract type returned by cftime, which triggers type-inference explosion
    T_time = eltype(nctimes)
    starttime = reinterpret(T_time, DateTime(string(config.time.starttime)))
...
function run!(model::Model; close_files = true)
    ...
    dt = clock.dt
    # Fix 7 (ARM JIT): cftime() returns abstract UnionAll type, causing type instability.
    # Use last element of forcing times as endtime (same concrete type as starttime).
