...
function Model(config::Config, type::SbmModel)
    ...
    # Fix 6 (ARM JIT): bypass Clock(config, reader) which triggers a 30+ min LLVM
    # compilation cascade via CFTime constructors. Use nctimes[1] directly as
    # the concrete starttime; all three config fields are set in our TOML.
