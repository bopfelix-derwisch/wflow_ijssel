...
# Type-stable variants: return Val instances so Period(Tdata, tuf, ::Val{F}, ::Val{E}) dispatch works
@inline _itype(::Type{Period{T, Tfactor, Texponent}}) where {T, Tfactor, Texponent} = T
# Tfactor may already be a Val instance (e.g. Val{1}()) or a Val type (Val{1}); handle both
@inline _factor_type(::Type{Period{T, Tfactor, Texponent}}) where {T, Tfactor, Texponent} = isa(Tfactor, Val) ? Tfactor : Tfactor()
@inline _exponent_type(::Type{Period{T, Tfactor, Texponent}}) where {T, Tfactor, Texponent} = isa(Texponent, Val) ? Texponent : Texponent()

# Type-stable Period constructor: use instance Val{F}()/Val{E}() as type params to match
# the outer CFDateTime period type which uses instance type parameters
