...
for (CFDateTime, calendar) in [
    ...
    @eval begin
        function $CFDateTime{T, Torigintuple}(args::Vararg{Integer, N}) where {T, Torigintuple, N}
            ...
            DT = $CFDateTime
            # Use _factor_type/_exponent_type/_itype to keep Val types (not unwrapped integers)
            # so the Period constructor can infer a concrete return type — avoids ARM JIT explosion
            Tfactor = _factor_type(T)
            Texponent = _exponent_type(T)
            Ti = _itype(T)
